...
{{-- Content --}}
 <div class="sm:ml-64">
    ...
    {{-- End Section 1 --}}
    {{-- Section 2 --}}
    <form action="{{ route('client.edit-authors', ['id' => $authors->id]) }}" method="POST" class="m-0 p-0">
        @method('PUT')
        @csrf
        <div class="bg-white">
            {{-- Nama Pengarang --}}
                <div class="flex border-y border-solid border-gray-300">
                    ...
                    <div class="px-4 py-3">
                        <input type="text" id="small-input" value="{{ $author->title}}" class="w-96 py-1 px-2 text-gray-900 border rounded text-sm border-solid border-gray-400 focus:ring focus:ring-blue-300">
                    </div>
                </div>
            {{-- End Nama Pengarang --}}
