    ...
    <div class="mt-18 mb-4">
        ...
        <div id="session" class="py-3 px-4 bg-blue-100">
            <div class="p-4 bg-red-600">
                <p class="text-white">{{ session('destroy') }}</p>
            </div>
        </div>
        @endif
        {{-- End Succes message --}}
        ...
        {{-- End Section 2 --}}
        {{-- Section 3 --}}
        <form action="{{ route('client.delete-eksemplar') }}" method="POST">
            @csrf
            @method('DELETE')
                            ...
                            <tr class="border-b border-solid border-gray-400">
                                <td class="p-3 w-16">
                                    <div class="flex items-center justify-center">
                                        <input id="default-checkbox" name="deletedEksemplar[]" type="checkbox"
                                            value="{{ $eks->id }}" class="w-4 h-4  border-black rounded">
                                    </div>
                                </td>
                                <td class="p-3 w-20">
