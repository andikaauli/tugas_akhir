    ...
    <div class="">
        <div class="grid gap-10 grid-cols-3 px-20 mb-5">
            <div class="col-span-2">
                <div
                    class="bg-teknik border-x border-t border-black rounded-t-md py-5 text-xl font-extrabold text-white text-center">
                ...
                <div class="px-12 py-10 border-x border-b border-black rounded-b-md">
                    <div class="flex w-full">
                        ...
                        <div class=" ml-4">
                            ...
                            @if (isset($bibliografi->author->title))
                                <p class="text-xl font-bold text-slate-600">{{ $bibliografi->author->title }}</p>
                            @else
                                <p class="text-xl font-bold text-slate-600">-</p>
                            @endif
                        </div>
                    </div>
                    <div class="grid gap-3 grid-cols-2">
                        ...
                        <div class="col-span-1">
                            ...
                            @if ($bibliografi->classification)
                                <p class="text-start mb-4 font-semibold text-base">{{ $bibliografi->classification }}
                                </p>
                            @else
                                <p class="text-start mb-4 font-semibold text-red-500 text-base">Tidak Ada Data</p>
                            @endif
                            @if ($bibliografi->publisher->title)
                                <p class="text-start mb-4 font-semibold text-base">{{ $bibliografi->publisher->title }}
                                </p>
                            @else
                                <p class="text-start mb-4 font-semibold text-red-500 text-base">Tidak Ada Data</p>
                            @endif
                                ...
                                <div class="border-t border-black w-fit">
                                    @foreach ($bibliografi->eksemplar as $eks)
                                        <div class="flex border-b border-black w-fit">
                                            <div class="border-x px-2 py-1.5 border-black w-24">{{ $eks->item_code }}
                                            </div>
                                            <div class="border-r px-2 py-1.5 border-black">
                                                {{ $bibliografi->call_number }}</div>
                                            <div class="border-r px-2 py-1.5 border-black w-24">
                                                {{ $eks->bookstatus->name }}</div>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                <p class="text-start mb-4 font-semibold text-red-500 text-base">Tidak Ada Data Eksemplar
                                </p>
                            @endif
                        </div>
                    </div>
            ...
            <div class="h-fit">
                ...
                <div class="py-4 px-6 border-x border-b border-black rounded-b-md">
                    <p class="font-medium text-lg text-justify">Akses Katalog Publik Daring - Gunakan fasilitas
                        pencarian untuk mempercepat penemuan data katalog
                    </p>
                </div>
            </div>
        </div>
